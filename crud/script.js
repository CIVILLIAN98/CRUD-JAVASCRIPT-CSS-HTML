// data
let data = [
  { id: 1, hid: false, name: "SHAXBOZ", Occupation: "MENTOR" },
  { id: 2, hid: false, name: "WAXZODBEK", Occupation: "developer" },
  { id: 3, hid: false, name: "ASRORBEK", Occupation: "developer" },
...
function saveFN(id) {
  data = data.map((v) => ({
    id: v.id,
    hid: false,
    name: v.id === id ? namechanger : v.name,
    Occupation: v.id === id ? occupationchanger : v.Occupation,
  }));
  render();
}
...
function render() {
  mainTag.innerHTML = data
    .map(
      (value) =>
        value.name.toLocaleLowerCase().includes(searcher.toLocaleLowerCase()) &&
        `
<div class ="section">
    <div>
        ID: ${value.id}
    ...
    <div>
        ${
          value.hid
            ? `<input type="text" value="${namechanger}" id="changeinputname"></input>`
            : `${value.name}`
        }
    </div>
    <div>
        ${
          value.hid
            ? `<input type="text" value="${occupationchanger}" id="changeinputoccupation"></input>`
            : `${value.Occupation}`
        }
    </div>
    <div>   
...
`
  ...
  const changeinputoccupation = document.getElementById(
    "changeinputoccupation"
  );
